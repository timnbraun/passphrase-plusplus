...
DICTS = [ '/usr/share/hunspell', '/usr/share/myspell' ]
ALTCHARS = '-+_%'

parser = argparse.ArgumentParser(description='Generate some random passwords')
parser.add_argument('--numbers', help='l33t numbers',
	action='store_true', default=True)
parser.add_argument('-n', '--no-numbers', help='no l33t numbers',
	action='store_false', dest='numbers')
parser.add_argument('--caps', help='Capitalization',
	action='store_true', default=True)
parser.add_argument('-c', '--no-caps', help='no capitalization',
	action='store_false', dest='caps')
parser.add_argument('--altchars', help='alternate characters',
	action='store_true', default=True)
parser.add_argument('-a', '--no-altchars', help='no alternate characters',
	action='store_false', dest='altchars')
parser.add_argument('--count', help='how many passwords do you want',
	default=5, type=int)
...
parser.add_argument('-l', '--lang', help='language the password is base on',
	choices=[ 'en_CA', 'fr_CA', 'sw_TZ' ], default='en_CA')
args = parser.parse_args()

def get_dict(lang):
	for dic_path in DICTS:
		dict_file = os.path.join( dic_path, lang + '.dic' )
		if os.path.exists( dict_file ):
			return dict_file
	print('Dictionary for', lang, 'not found')
	exit(2)

if (args.word_min > args.word_max):
	print('Impossible options, word_min', args.word_min,
		'> word_max', args.word_max)
	exit(1)

if (args.phrase_min > args.phrase_max):
	print('Impossible options, phrase_min', args.phrase_min,
		'> phrase_max', args.phrase_max)
	exit(1)

with open(get_dict(args.lang), 'r') as file:
...
def phrase(altchars, caps):
	...
	while (len(p) < args.phrase_min):
		...
		if ((((args.phrase_min - len(p)) < args.word_min) and
			...
			# start over
			# print('reset: min', args.phrase_min,
			#		'len(p)', len(p), 'word_min', args.word_min)
...
for i in range(args.count):
	print( l33tphrase(args.altchars, args.caps)
		if args.numbers else phrase(args.altchars, args.caps) )
